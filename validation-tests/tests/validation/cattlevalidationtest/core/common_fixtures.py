...
template_dir = os.path.join(os.path.split(os.path.split(os.path.split(os.path.abspath(__file__))[0])[0])[0],
                            "cattlevalidationtest", "data", "compose")

srv_res_test_rancher_compose_template = 'rancher-compose-healthcheck-service-resiliency.yml'
srv_res_test_docker_compose_template = 'docker-compose-healthcheck-service-resiliency.yml'
lb_healthcheck_test_rancher_compose_template = 'rancher-compose-lb-targets-healthcheck-enabled.yml'
lb_healthcheck_test_docker_compose_template = 'docker-compose-lb-targets-healthcheck-enabled.yml'
...
def run_rancher_compose(client, srv_res_test_rancher_compose_template, srv_res_test_docker_compose_template):
    access_key = client._access_key
    secret_key = client._secret_key
    docker_filename = os.path.join(template_dir, srv_res_test_docker_compose_template)
    rancher_filename = os.path.join(template_dir, srv_res_test_rancher_compose_template)

    project_name = "rancher-test"
    ...
    cmd5 = "./rancher-compose -p " + project_name + " -f " + 'docker-compose.yml' + \
           " -r " + 'rancher-compose.yml' + " up -d"
    ...
    with SCPClient(ssh.get_transport()) as scp:
        scp.put(docker_filename, '/root/rancher-compose-v0.2.5/docker-compose.yml')
        scp.put(rancher_filename, '/root/rancher-compose-v0.2.5/rancher-compose.yml')
...
def load_compose_template(rancher_compose_template, docker_compose_template):
    r = yaml.safe_load(open(os.path.join(template_dir, srv_res_test_rancher_compose_template)))
    d = yaml.safe_load(open(os.path.join(template_dir, srv_res_test_docker_compose_template)))
    print r, "\n\n"
    print d, "\n\n"
    return r, d